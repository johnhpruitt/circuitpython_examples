...
# (130, not 132 or 128, since it is upside down)
...
wifi_coprocessor = False
if os.getenv("WIFI_ESP01_COPROCESSOR") == 'True':
    if len(conections_dict) > 0:
        ...
        if wifi_ssid is not None and wifi_password is not None:
            ...
            wifi_coprocessor = True
            footer_text = "Looking for WIFI"
            footer_text_area.text = footer_text

if wifi_coprocessor:
    from esp01_serial_wrapper import ESP01wrapper
    uart = busio.UART(board.GP8, board.GP9, baudrate=115200)
    esp01 = ESP01wrapper(uart, conections_dict, wifi_json)
    esp01.search_stations(1000, 5)
    ...
    try:
        ...
        footer_text = "NO WIFI MODULE"
        footer_text_area.text = footer_text

    print("conections_dict:")
    print(conections_dict)
    print("wifi_json:")
    print(wifi_json)
    """
